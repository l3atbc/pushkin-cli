...
module.exports = class WorkerConstructor {
  constructor(name) {
    this.dockerPaths = {};
    this.name = name;
  ...
  loadOriginalDocuments() {
    ...
    this.dockerPaths.normal.original = yaml.safeLoad(
      fs.readFileSync(this.getDockerPath('normal'), 'utf-8')
    );
  ...
  writeDocuments() {
    Object.keys(this.dockerPaths).map(key => {
      fs.writeFileSync(
        ...
        yaml.safeDump(this.dockerPaths[key].document, { noRefs: true })
      );
    });
  }
  copyFolder() {
